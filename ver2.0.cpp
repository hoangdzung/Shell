...
void Suspend_process(DWORD  dwProcessId);
void Resume_process(DWORD  dwProcessId);
void Kill_process(DWORD  dwProcessId);
    ...
    cout << "Error";
  	...
  	do { 
    	if(te32.th32OwnerProcessID == dwProcessId) {
		    ...
		    HANDLE current_Handle = OpenThread((THREAD_SUSPEND_RESUME, THREAD_TERMINATE), TRUE, te32.th32ThreadID);
		    SuspendThread(current_Handle);
		    // printf( TEXT("\n     Handle         = %p"), a ); 
    		// CloseHandle(a);
		}
	...
	for(int i = process_list.size() - 1; i >= 0; i--) {
		if (process_list[i].name == file && process_list[i].status == 1) {
			...
			if (action == "kill") {
				...
				Suspend_process(process_list[i].pi.dwProcessId);
			else if (action == "resume")
				// ResumeThread(process_list[i].pi.hThread);
				Resume_process(process_list[i].pi.dwProcessId);
			break;
		}
	}
	if (!found_flag) 
